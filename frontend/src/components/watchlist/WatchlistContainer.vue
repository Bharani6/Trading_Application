    ...
    <div class="trade-modal-overlay" v-if="showModal" @click.self="closeModal">
      <div class="trade-modal-card">
        ...
        <div class="modal-body">
          ...
          <div v-if="modalQuantity > 0 && modalAction === 'Buy' && (modalQuantity * modalStock.currentPrice) > walletStore.state.balance.available_balance" style="color: var(--text-muted); font-size: 0.85rem; margin-top: 0.5rem; text-align: right;">
            Available amount is enough for {{ Math.floor(walletStore.state.balance.available_balance / modalStock.currentPrice) }} shares
          </div>
        </div>
